...
async fn try_or_terminate_session<T>(res: Result<T, SSMError>, client: &Client, session_id: Option<String>) -> Result<T, SSMError> {
    match res {
...
pub async fn launch_ssm_tunnel(
    ...
    let mut tunnel_child = try_or_terminate_session(
        initiate_ssm_port_forwarding(&start_session_output).await, &aws_client, start_session_output.session_id.clone()
    ).await?;

    let stdout = match tunnel_child.stdout.take() {
        Some(c) => c,
        _ => {
    ...
    receive_or_terminate_session(rx_app_exit.await, &aws_client, start_session_output.session_id.clone()).await?;

    send_log("SSM Tunnel : stop app received".into(), &logs_sender);

    send_or_terminate_session(tx_app_exit_ack, ApplicationExitedMessage, &aws_client, start_session_output.session_id.clone()).await?;
...
async fn receive_or_terminate_session<T>(res: Result<T, RecvError>, client: &Client, session_id: Option<String>) -> Result<(), SSMError> {
    let recv_res = receive(res);
    match recv_res {
        Err(e) => terminate_session_with_error(
            e,
            client,
            session_id
        ).await,
...
fn output_tunnel(buf_reader: &mut BufReader<std::process::ChildStdout>, logs_sender: &std::sync::mpsc::Sender<String>) -> Result<(), SSMError> {
...
async fn terminate_session_with_error<T>(err: SSMError, client: &Client, session_id: Option<String>) -> Result<T, SSMError> {
